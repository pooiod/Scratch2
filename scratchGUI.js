...
.s2-card.laggy { border-color: #ccb63a; }
...
function showProjectPicker(willreload) {
    ...
    function addCard(p) {
        ...
        var str = p.description.toLowerCase() + p.instructions.toLowerCase();
        var lagwarn = [
            "phosphorus",
            "forkphorus",
            "turbowarp",
            ...
            if (Array.isArray(rule)) {
                return rule.every(word => str.includes(word))
            }
            return str.includes(rule)
        });

        if (lagwarn) card.classList.add("laggy");
    ...
    function load(reset) {
        ...
        var api = query
            ...
            : "https://scratch.pooiod7.workers.dev/users/S2Listing/favorites?q=i3ow&mode=recent&limit=" + limit + "&offset=" + (page * limit); // /explore/projects
