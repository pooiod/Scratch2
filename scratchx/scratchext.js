...
Scratch.extensions = {
    unsandboxed: true,
    register: function (extensionInstance) {
        ...
        const convertLabel = (text, args) => {
            let argOrder = [];
            let s2Text = text.replace(/\[([^\]]+)\]/g, (match, argName) => {
                argOrder.push(argName);
                const arg = args[argName];
                if (arg && (arg.type === Scratch.ArgumentType.NUMBER || arg.type === Scratch.ArgumentType.ANGLE)) {
                    return '%n';
                }
                if (arg && arg.type === Scratch.ArgumentType.BOOLEAN) {
                    return '%b';
                }
                return '%s';
            });
        ...
        info.blocks.forEach(block => {
            const { s2Text, argOrder } = convertLabel(block.text, block.arguments || {});
            
            let s2Type = ''; 
            if (block.blockType === Scratch.BlockType.REPORTER) s2Type = 'r';
            else if (block.blockType === Scratch.BlockType.BOOLEAN) s2Type = 'b';
            else if (block.blockType === Scratch.BlockType.HAT) s2Type = 'h';
            else {
                s2Type = 'w'; 
            }

            const s2Block = [s2Type, s2Text, block.opcode];
            argOrder.forEach(argName => {
                s2Block.push(block.arguments[argName].defaultValue || '');
            });
            s2Descriptor.blocks.push(s2Block);

            s2Ext[block.opcode] = function () {
                const argsArray = Array.from(arguments);
                let callback = null;

                if (s2Type === 'w' || s2Type === 'r' || s2Type === 'b') {
                    callback = argsArray.pop();
                }

                const s3Args = {};
                argOrder.forEach((name, index) => {
                    s3Args[name] = argsArray[index];
                });

                // Mock S3's "util" object
                const util = {
                    yield: () => { /* Scratch 2 handles yielding via the callback */ }
                };

                const result = extensionInstance[block.opcode](s3Args, util);
